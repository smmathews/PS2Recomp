...
namespace ps2_stubs
{
    ...
    namespace
    {
        ...
        // ---- degenerate-matrix probe (PS2X_VU0TRACE=1) -----------------------
        // Answers "which sceVu0 call produced the all-zero / all-NaN transform
        // that reaches VU1, and who called it". Off unless the env var is
        // exactly "1"; capped so it can never become a firehose.
        bool vu0TraceOn()
        {
            static const bool on = []()
            {
                const char *e = std::getenv("PS2X_VU0TRACE");
                return e && e[0] == '1' && e[1] == '\0';
            }();
            return on;
        }

        bool matrixIsZero3x3(const float (&m)[16])
        {
            for (int r = 0; r < 3; ++r)
                for (int c = 0; c < 3; ++c)
                    if (m[4 * r + c] != 0.0f)
        ...
        bool matrixHasNaN(const float (&m)[16])
        {
            for (int i = 0; i < 16; ++i)
                if (std::isnan(m[i]))
                    return true;
        ...
        void vu0TraceMatrix(const char *what, R5900Context *ctx, uint32_t srcAddr,
                            uint32_t dstAddr, const float (&m)[16])
        {
            if (!vu0TraceOn())
                return;
            const bool zero = matrixIsZero3x3(m);
            const bool nan = matrixHasNaN(m);
            if (!zero && !nan)
                return;
            // One line per (call-site, kind) pair, then a global cap.
            static std::mutex mu;
            static std::unordered_set<uint64_t> seen;
            static uint32_t emitted = 0;
            const uint32_t ra = ctx ? getRegU32(ctx, 31) : 0u;
            const uint64_t key = (static_cast<uint64_t>(ra) << 8) |
                                 (zero ? 1u : 0u) | (nan ? 2u : 0u);
            std::lock_guard<std::mutex> lk(mu);
            if (emitted >= 64u || !seen.insert(key).second)
                return;
            ++emitted;
            std::cerr << "[vu0:degen] " << what << (zero ? " ZERO3x3" : "")
                      << (nan ? " NaN" : "") << " ra=0x" << std::hex << ra
                      << " src=0x" << srcAddr << " dst=0x" << dstAddr << std::dec;
            for (int i = 0; i < 16; ++i)
                std::cerr << (i % 4 == 0 ? " | " : " ") << m[i];
            std::cerr << std::endl;
        }
        ...
        void rigidInverse(const float (&in)[16], float (&out)[16])
        {
            ...
            // Hardware finishes with `vsub.xyz vf04, vf09, vf04` where vf09.xyz
            // is +0, i.e. it computes (0 - dot), not (-dot). The two differ for
            // a zero dot: 0-0 is +0.0 but -0.0f is -0.0. Matching the subtract
            // keeps a degenerate input from being labelled with a -0.0 that the
            // console would never produce.
            for (int col = 0; col < 3; ++col)
                out[12 + col] = 0.0f - ((tx * in[4 * col]) + (ty * in[4 * col + 1]) + (tz * in[4 * col + 2]));
            out[15] = in[15];
        }
    ...
    void sceVu0CameraMatrix(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        if (readVuVec4f(rdram, eyeAddr, eye) && readVuVec4f(rdram, fwdAddr, fwd) && readVuVec4f(rdram, upAddr, up))
        {
            ...
            // CameraMatrix calls straight through to sceVu0Normalize on
            // hardware, so this must be the same XYZ-only normalize with a
            // zeroed w -- see the note on sceVu0Normalize below.
    ...
    void sceVu0InnerProduct(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        if (readVuVec4f(rdram, lhsAddr, lhs) && readVuVec4f(rdram, rhsAddr, rhs))
        {
            // `vmul.xyz` then `vaddy.x` + `vaddz.x`: a 3-component dot. The w
            // lane is never multiplied in, so w must not contribute here.
    ...
    void sceVu0InversMatrix(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        {
            // The degenerate view matrix that reaches VU1 qw24-27 is this
            // function's output, so trace the INPUT: a zero/NaN 3x3 here means
            // the defect is upstream in the guest code that built the matrix.
            vu0TraceMatrix("InversMatrix.in", ctx, srcAddr, dstAddr, in);
            rigidInverse(in, out);
            (void)writeVuMatrix4f(rdram, dstAddr, out);
        }
        setReturnS32(ctx, 0);
    ...
    void sceVu0MulMatrix(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        {
            // out = m1 . m0, second source on the LEFT. libvu0 loads the four
            // rows of the *first* source (a1) into vf04..vf07 once, then walks
            // the *second* source (a2) a row at a time using each row as the
            // broadcast coefficients:
            //   out_row_i = a2[i].x*a1row0 + a2[i].y*a1row1 + ... + a2[i].w*a1row3
            // which is out[i][j] = sum_k a2[i][k] * a1[k][j], i.e. a2 . a1.
            // (The same idiom in RotMatrixZ puts the *rotation* in the vf04..07
            // slot and the incoming matrix in the walked slot, giving src . R --
            // so the two are consistent; only the argument naming differs.)
            mulVuMatrix(m1, m0, out);
            vu0TraceMatrix("MulMatrix.out", ctx, m0Addr, dstAddr, out);
            (void)writeVuMatrix4f(rdram, dstAddr, out);
        }
        setReturnS32(ctx, 0);
    }
    ...
    void sceVu0Normalize(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        if (readVuVec4f(rdram, srcAddr, src))
        {
            // libvu0's Normalize is XYZ-only in both halves: the length comes
            // from `vmul.xyz` + two `vadd*.x` (so w never enters it), and the
            // scale is applied by `vmulq.xyz` into a register that was just
            // cleared by `vsub.xyzw`, so the stored w is 0 -- not src.w scaled.
            // Including w in the length is badly wrong for the common case of a
            // direction vector carrying a non-zero w, and shrinks the result
            // toward zero without bound as w grows.
