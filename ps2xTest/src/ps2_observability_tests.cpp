...
//
// Statics caveat: should_log()/ps2_watch registry state are process-lifetime
// (see runtime/ps2_diag.h, runtime/ps2_guestwatch.h), and the GS clut-dump
// "seen keys" set / [gs:image] dbp-change tracker are function-local statics
// inside ps2xRuntime/src/lib/ps2_gs_rasterizer.cpp and ps2_gs_gpu.cpp. Every
// test below is written to be robust to running in any order relative to the
// rest of the suite: watch tests fully reset via clearWatches(), CLUT tests
// use a distinct cbp per case so they always hit a fresh dump, and the
// [gs:image] test picks a DBP value that is essentially guaranteed to differ
// from whatever the previous transfer's DBP was (the probe logs
// unconditionally whenever DBP changes). Every test leaves diagnostics
// disabled, watches cleared, and the log ring cleared on exit.
