...
namespace
{
    ...
    void cleanupRuntime(TestEnv &env)
    {
        ...
        notifyRuntimeStop();
        // Belt-and-suspenders isolation. The suite's actual order-independence comes
        // from main.cpp's BeforeEach(reset_ps2_test_function_table), which wipes the
        // recompiled-function table between tests; disabling the reset below still
        // yields a green suite. This reset additionally clears the INTC-specific
        // global state (handler tables + enable masks + pending latch are global, not
        // per-TestEnv) as forward-defensive cleanup so each test starts from a known
        // INTC state.
